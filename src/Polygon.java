...
public class Polygon {
	...
	double perimeter(List<point> P) {
		double result = 0.0;
		for (int i = 0; i < (int) P.size() - 1; i++) // remember that P[0] =
														// P[n-1]
	...
	List<point> cutPolygon(point a, point b, List<point> Q) {
		...
		for (int i = 0; i < (int) Q.size(); i++) {
			...
			if (left1 > -EPS)
				P.add(Q.get(i)); // Q[i] is on the left of ab
	...
	List<point> CH(List<point> P) {
		...
		Collections.sort(P, new Comparator<point>() {
			public int compare(point a, point b) { // angle-sorting function
				if (collinear(pivot, a, b))
					return dist(pivot, a) < dist(pivot, b) ? -1 : 1; // which
																		// one
																		// is
																		// closer?
		...
		while (i < n) { // note: n must be >= 3 for this method to work
			...
			if (ccw(S.get(j - 1), S.get(j), P.get(i)))
				...
				S.remove(S.size() - 1); // or pop the top of S until we have a
										// left turn
		}
		return S;
	} // return the result
